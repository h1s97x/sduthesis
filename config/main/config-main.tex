...
% -------------------算法设置-------------------
% algorithmicx 宏包内部命令，无需手动调用
\newcommand{\Return}{\State \textbf{返回:\ }}
...
% 检查字体文件是否存在
% 如果 assets/fonts/ 目录下有字体文件，使用本地字体；否则使用系统字体

% 字体文件检测命令
\def\IfFontExists#1#2#3{
  \IfFileExists{#1}{#2}{#3}
}

% 中文字体配置（跨平台支持）
\IfFontExists{assets/fonts/SimSun.ttc}{%
  % 使用本地字体（Windows/macOS）
...
% 设置英文字体（跨平台支持）
\IfFontExists{assets/fonts/times.ttf}{%
  \setmainfont{Times New Roman}
}{%
  % 使用 TeX Gyre Termes 作为 Times New Roman 的开源替代
...
% 设置数学模式下的字体
\IfFontExists{assets/fonts/xits-math.otf}{%
  \setmathfont{XITS Math}
  \setmathfont[range=\mathop]{Latin Modern Math}
}{%
  % 使用 Latin Modern Math 作为默认数学字体
